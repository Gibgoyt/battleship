  ...
  return (
    <div class="p-8 space-y-8">
      ...
      <div class="border-b pb-4" style={props.isDark ? 'border-color: #27272a;' : 'border-color: #e5e7eb;'}>
        <h1 class={`text-3xl font-semibold ${props.isDark ? 'text-white' : 'text-gray-900'}`}>
          SPLITDO Token Presale
        </h1>
        <p class={`mt-2 text-sm ${props.isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Manage your wallet and purchase SPLITDO tokens
        </p>
      </div>

      {/* Connection Status - Clean Banner */}
      ...
      <Show when={connectionStatus() === 'connected' && wallet()}>
        <div class={`p-3 border-l-4 ${props.isDark ? 'bg-zinc-800/50 border-green-500 text-green-400' : 'bg-green-50 border-green-500 text-green-800'}`}>
          <span class="font-medium">Connected to {wallet()?.name}:</span> {formatAddress(wallet()?.address || '')}
        </div>
      </Show>

      {/* Balance Overview - Professional Table */}
      <div>
        <h2 class={`text-xl font-semibold mb-4 ${props.isDark ? 'text-white' : 'text-gray-900'}`}>
          Account Balances
        </h2>
          ...
          <table class={`w-full border-collapse ${props.isDark ? 'border-zinc-700' : 'border-gray-200'}`} style="border-width: 1px;">
            <thead>
              <tr class={props.isDark ? 'bg-zinc-800/50' : 'bg-gray-50'} style={props.isDark ? 'border-bottom: 1px solid #3f3f46;' : 'border-bottom: 1px solid #e5e7eb;'}>
                <th class={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${props.isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  Asset
                </th>
                <th class={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider ${props.isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                  Balance
            ...
            <tbody>
              ...
              <tr class={props.isDark ? 'border-zinc-700' : 'border-gray-200'} style="border-bottom-width: 1px;">
                <td class={`px-6 py-4 whitespace-nowrap font-medium ${props.isDark ? 'text-white' : 'text-gray-900'}`}>
                  SPLITDO Token
                </td>
                <td class={`px-6 py-4 whitespace-nowrap ${props.isDark ? 'text-gray-300' : 'text-gray-700'}`}>
